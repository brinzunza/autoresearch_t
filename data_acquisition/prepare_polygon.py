...
Usage:
    python3 data_acquisition/prepare_polygon.py                    # use latest data
    python3 data_acquisition/prepare_polygon.py --ticker AAPL      # specify ticker
    python3 data_acquisition/prepare_polygon.py --csv path/to/data.csv  # use specific CSV
...
def normalize_ticker(ticker):
    """
    Normalize ticker to clean format for file lookups.
    ...
    if ticker is None:
        return None
    return ticker.replace('C:', '').replace('/', '').replace('_', '').upper()
...
def load_polygon_data(csv_path=None, ticker=None):
    ...
    Args:
        ...
        ticker: Ticker symbol in any format (EUR/USD, C:EURUSD, EURUSD, etc.)
...
def save_prepared_data(df, ticker, symbol_name=None):
    ...
    Args:
        ...
        symbol_name: Optional name to use for file (defaults to normalized ticker)
    """
    os.makedirs(DATA_DIR, exist_ok=True)

    if symbol_name is None:
        # Normalize ticker for consistent filenames
        # EUR/USD -> EUR_USD to match prepare.py convention
        symbol_name = ticker.replace('C:', '').replace('/', '_')
...
def main():
    ...
    try:
        ...
        print("  python3 data_acquisition/fetch_data.py")
        sys.exit(1)
    ...
    print(f"     python3 train.py --symbols {args.save_as or ticker}")
    print()
